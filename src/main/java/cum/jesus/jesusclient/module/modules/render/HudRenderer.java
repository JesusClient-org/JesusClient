...
public class HudRenderer implements IRenderer {
    ...
    @Override
    public void drawRect(int mode, double x, double y, double w, double h, int c) {
        RenderUtils.drawRect(mode, x / 2.0, y / 2.0, x / 2.0 + w / 2.0, y / 2.0 + h / 2.0, c);
    }
    ...
    public void drawString(int x, int y, String text, int color) {
        renderer.drawString(text, x / 2f, y / 2f, color, false);
    }
